  ...
  return (
    <nav className='flex items-center bg-white py-3 px-4 dark:bg-zinc-800 justify-between border-b-1 border-gray-200 dark:border-zinc-700'>
         ...
         <div className='flex gap-3 items-center relative'>
              ...
              <button title='toggle view' onClick={()=>dispatch(toggleView())} className='text-gray-600 hover:bg-gray-100 h-fit hover:dark:bg-zinc-700 rounded-4xl transition-all duration-150 p-2 sm:inline hidden dark:text-zinc-400'>{view==='ROWS'?<LayoutGrid size={18}/>:<Rows3 size={18} />}</button>
              <button ref={filterButtonRef} onClick={togglePopup} title='Filters' className='text-gray-600 hover:bg-gray-100 h-fit hover:dark:bg-zinc-700 rounded-2xl transition-all duration-150 p-2 inline dark:text-zinc-400'><SlidersHorizontal size={18} /></button>
