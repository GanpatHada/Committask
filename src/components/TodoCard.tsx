...
import { getDaysRemains, isTaskDeadlinePassed } from "@/utils/dateHelper";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-center">
        ...
        <p className="leading-tight font-semibold text-gray-600 text-md dark:text-zinc-200 group-hover:text-white">
          <span>{title}</span>
          <span
            title={capitalize(priority) + " priority"}
            className={`${getPriorityColor(priority as Priority)} relative top-0.5 inline-flex items-center text-sm rounded-xl ml-1`}
          >
            <RiFlag2Fill />
          </span>
        </p>
        ...
        <div className='text-white rounded-md scale-0 opacity-0 transition-all duration-100 delay-150 ease-out gap-2 group-hover:scale-100 group-hover:opacity-100 flex absolute right-2 bottom-2'>
         {!isTaskDeadlinePassed(todo.dueDate) && <button onClick={handleEditTodo} className="bg-gray-100/20 hover:bg-purple-600 p-2 rounded-2xl" title='Edit task' ><Pencil size={16} /></button>}
